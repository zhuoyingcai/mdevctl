...
impl Callout {
    ...
    pub fn get_attributes_helper(dev: &mut MDev) -> Result<serde_json::Value> {
        let event = Event::Get;
    ...
    pub fn get_attributes(dev: &mut MDev) -> serde_json::Value {
        match Callout::get_attributes_helper(dev) {
            Ok(j) => j,
